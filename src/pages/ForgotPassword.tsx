...
const ForgotPassword = () => {
  ...
  const requestReset = async (resent = false) => {
    if (!validate()) return;
    setLoading(true);
    setResentNotice(false);
    try {
      await authApi.requestPasswordReset(email.trim(), `${window.location.origin}/reset-password`);
      setSent(true);
      setResentNotice(resent);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen grid md:grid-cols-2">
      <div className="relative hidden md:block">
        <img src={hero} alt="Worship gathering" className="absolute inset-0 h-full w-full object-cover" />
          ...
          {!sent ? (
            <>
              ...
              <p className="mt-3 text-sm text-muted-foreground leading-relaxed">
                Enter the email address linked to your NoraPlus account and we will send you a secure reset link.
              </p>
              ...
              <form onSubmit={handleSubmit} noValidate className="mt-8 space-y-5">
                <div>
                  ...
                  {error && <p className="mt-2 text-xs text-red-soft">{error}</p>}
                </div>
              ...
              <p className="mt-4 text-center text-xs text-muted-foreground">Your reset link will expire for your security.</p>
            ...
            <>
              ...
              <h2 className="mt-6 text-center font-display text-3xl">Check Your Email</h2>
              ...
              {resentNotice && <p className="mt-4 text-center text-xs text-gold">Reset link resent successfully.</p>}
              {error && <p className="mt-4 text-center text-xs text-red-soft">{error}</p>}

              <div className="mt-8 flex flex-col items-center gap-4">
                <Link to="/auth" className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-red-gradient py-3 text-sm font-medium text-primary-foreground shadow-red-glow transition hover:shadow-glow">
                  Back to Sign In
                </Link>
                <button onClick={() => requestReset(true)} disabled={loading} className="text-sm text-gold hover:underline disabled:opacity-60 disabled:cursor-not-allowed">
