        ...
        activePlaylist ? (
          <PlaylistDetail
            ...
            onRemoveItem={(contentId) =>
              removeFromPlaylist.mutate({ playlistId: activePlaylist.id, contentId }, {
                onSuccess: () => {
...
const PlaylistsGrid = ({
  playlists, isLoading, onOpen, onDelete,
}: {
    ...
    return (
      <div className="rounded-2xl border border-dashed border-border p-12 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Tap "Add to Playlist" on any content to create your first playlist.
        </p>
      </div>
    );
  }
          ...
          <button className="w-full text-left p-5" onClick={() => onOpen(p)}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{p.items.length} {p.items.length === 1 ? "item" : "items"}</p>
              </div>
            </div>
            {p.items.length > 0 && (
              <div className="flex gap-1.5">
                {p.items.slice(0, 3).map((item) => (
                  item.content.thumbnailUrl ? (
                    <img key={item.id} src={item.content.thumbnailUrl} alt=""
                      className="h-12 w-12 rounded-lg object-cover ring-1 ring-border/40" />
                  ) : (
...
const PlaylistDetail = ({
  ...
  <div className="space-y-6">
    <div className="flex items-center gap-3">
      <button onClick={onBack} className="text-sm text-gold hover:underline">← All Playlists</button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {playlist.items.map((entry) => (
          <div key={entry.id} className="group relative">
            <ContentCard item={adaptContent(entry.content)} />
            <button
