...
const fallbackSlides: HeroSlide[] = [
  {
    ...
    image: message,
    primary: { label: "Play Now", to: "/app", icon: "play" },
    secondary: { label: "Browse Content", to: "/app" },
  },
  {
    id: "fallback-live",
    badge: "Live Event",
    ...
    image: event,
    primary: { label: "View Live", to: "/app/live", icon: "live" },
  ...
  },
  {
    ...
    image: devotional,
    primary: { label: "Listen Now", to: "/app/devotionals", icon: "play" },
    secondary: { label: "Devotionals", to: "/app/devotionals" },
  },
  {
    id: "fallback-movie",
    badge: "Movie",
    title: "The Narrow Way",
    description: "A kingdom story about faith, choices, and transformation.",
    image: movie,
    primary: { label: "Watch Now", to: "/app/watch", icon: "play" },
    secondary: { label: "Watch", to: "/app/watch" },
  },
  {
    id: "fallback-worship",
    badge: "Worship",
    title: "Sounds of Worship",
    description: "Spirit-lifting worship for devotion, prayer, and reflection.",
    image: music,
    primary: { label: "Listen Now", to: "/app/listen", icon: "play" },
    secondary: { label: "Listen", to: "/app/listen" },
  },
];

export const HeroSlider = ({ slides = fallbackSlides }: { slides?: HeroSlide[] }) => {
  ...
  const go = useCallback(
    (n: number) => setI((activeSlides.length + n) % activeSlides.length),
    [activeSlides.length],
  );

  useEffect(() => {
    setI((current) => Math.min(current, activeSlides.length - 1));
  }, [activeSlides.length]);
